...
export default function FacultyDashboard() {
  ...
  // Get faculty_id from localStorage
  const facultyId = typeof window !== 'undefined' ? Number(localStorage.getItem('faculty_id')) || 1 : 1;
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const todayData = await getTodaySchedule(facultyId);
      const formattedToday = (todayData || []).map((event) => {
        const hour = 9 + (event.period_number - 1) * 2;
        return {
          id: event.id,
          time: `${String(hour).padStart(2, '0')}:00 AM`,
          lab: `Lab-${String.fromCharCode(64 + event.period_number)}`,
          batch: event.class_name,
          subject: event.subject_name,
          duration: '2 hrs',
          status: isEventOngoing(event.event_date, event.period_number) ? 'ongoing' : 'upcoming',
        };
      });
      setTodaySchedule(formattedToday);

      // Fetch upcoming exams
      const upcomingData = await getUpcomingEvents(facultyId);
      const formattedExams = (upcomingData || [])
        .filter((event) => event.event_type === 'LAB_EXAM')
        .map((event) => {
          const hour = 9 + (event.period_number - 1) * 2;
          return {
            id: event.id,
            date: formatDate(event.event_date),
            subject: event.subject_name + ' Exam',
            batch: event.class_name,
            lab: `Lab-${String.fromCharCode(64 + event.period_number)}`,
            time: `${String(hour).padStart(2, '0')}:00 AM`,
          };
      ...
      // Fetch notifications
      const notificationsData = await getNotifications(facultyId);
      const formattedNotifications = (notificationsData || []).map((notif) => ({
        id: notif.id,
        message: notif.message,
        time: timeAgo(notif.created_at),
        type:
          notif.notification_type === 'LAB_EXAM_SCHEDULED'
            ? 'success'
            : notif.notification_type === 'SLOT_REQUEST_STATUS'
              ? 'warning'
              : 'info',
      }));
  ...
  const isEventOngoing = (eventDate, periodNumber) => {
    const today = new Date().toISOString().split('T')[0];
    const currentHour = new Date().getHours();
    const periodStartHour = 9 + (periodNumber - 1) * 2;
    return eventDate === today && currentHour >= periodStartHour && currentHour < periodStartHour + 2;
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-gradient-to-br from-gray-100 to-pink-50">
        <div className="text-center">
          <BookOpen className="w-12 h-12 text-pink-500 mx-auto mb-4 animate-bounce" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 via-pink-50 to-blue-50 dark:bg-gradient-to-br dark:from-slate-900 dark:via-gray-900 dark:to-slate-950 p-4 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="relative z-10 flex items-center justify-between">
            <div>
              ...
              <p className="text-white/90 text-lg">
                You have {todaySchedule.length} lab sessions today • 18 hours this week
              </p>
            </div>
        ...
        <div className="bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50 shadow-xl">
          ...
          {todaySchedule.length === 0 ? (
            <p className="text-gray-600 dark:text-gray-400 text-center py-8">No sessions scheduled for today</p>
          ) : (
            <div className="space-y-4">
              {todaySchedule.map((session) => (
                <div
                  key={session.id}
                  className="group relative bg-gradient-to-r from-rose-50 via-pink-50 to-purple-50 dark:from-slate-800 dark:via-slate-800 dark:to-slate-800 p-6 rounded-2xl border-l-4 border-rose-500 hover:shadow-xl hover:shadow-pink-200/50 dark:hover:shadow-pink-900/50 transition-all"
                >
                      ...
                      <div className="flex-shrink-0">
                        <div
                          className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg ${
                            session.status === 'ongoing' ? 'bg-green-500' : 'bg-rose-500'
                          }`}
                        >
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50 shadow-xl">
            ...
            {upcomingExams.length === 0 ? (
              <p className="text-gray-600 dark:text-gray-400 text-center py-8">No exams scheduled</p>
            ) : (
              <div className="space-y-4">
        ...
        <div className="bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 border border-gray-200/50 dark:border-gray-700/50 shadow-xl">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="bg-red-500 text-white px-3 py-1 rounded-full text-sm font-bold">{notifications.length}</span>
          </div>
